...
import org.apache.hudi.common.testutils.HoodieTestUtils.{commitMetadataToBytArray, INSTANT_GENERATOR, TIMELINE_FACTORY}
  ...
  private def readArchivedInstantsBenchmark(): Unit = {
    withTempDir(f => {
      ...
      for (i <- 1 to commitsNum) {
        ...
        instantBuffer.add(new DummyActiveAction(instant, commitMetadataToBytArray(metadata)))
        if (i % batchSize == 0) {
