...
public class CommitMetadataSerDeV2 implements CommitMetadataSerDe {
  ...
  public Option<byte[]> serialize(org.apache.hudi.common.model.HoodieCommitMetadata commitMetadata) throws IOException {
    if (commitMetadata instanceof org.apache.hudi.common.model.HoodieReplaceCommitMetadata) {
      return getInstantWriter(MetadataConversionUtils.convertCommitMetadata(commitMetadata), HoodieReplaceCommitMetadata.class);
    }
    return getInstantWriter(MetadataConversionUtils.convertCommitMetadata(commitMetadata), HoodieCommitMetadata.class);
  }

  public static <T extends SpecificRecordBase> Option<byte[]> getInstantWriter(T metadata, Class<T> clazz)
      throws IOException {
