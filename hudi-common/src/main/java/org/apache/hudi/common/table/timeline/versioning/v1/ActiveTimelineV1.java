...
public class ActiveTimelineV1 extends BaseTimelineV1 implements HoodieActiveTimeline {
  ...
  public void saveAsComplete(HoodieInstant instant, Option<HoodieInstantWriter> writerOption) {
    ...
    transitionState(instant, instantGenerator.createNewInstant(HoodieInstant.State.COMPLETED, instant.getAction(), instant.requestedTime()), writerOption);
    LOG.info("Completed " + instant);
  }

  @Override
  public void saveAsComplete(boolean shouldLock, HoodieInstant instant, Option<HoodieInstantWriter> writerOption) {
    saveAsComplete(instant, writerOption);
  }
  ...
  public HoodieInstant transitionClusterRequestedToInflight(HoodieInstant requestedInstant, Option<HoodieInstantWriter> writerOption) {
    ...
    return transitionReplaceRequestedToInflight(requestedInstant, writerOption);
  }
  ...
  @Override
  public HoodieInstant transitionClusterInflightToComplete(boolean shouldLock, HoodieInstant inflightInstant, Option<HoodieInstantWriter> writerOption) {
    // In 0.x, no separate clustering action, reuse replace action.
    return transitionReplaceInflightToComplete(shouldLock, inflightInstant, writerOption);
  }
  ...
  protected void transitionState(HoodieInstant fromInstant, HoodieInstant toInstant, Option<HoodieInstantWriter> writerOption,
    ...
    try {
      ...
      if (metaClient.getTimelineLayoutVersion().isNullVersion()) {
        ...
        createFileInMetaPath(instantFileNameGenerator.getFileName(fromInstant), writerOption, allowRedundantTransitions);
        ...
        if (allowRedundantTransitions) {
          FileIOUtils.createFileInPath(storage, getInstantFileNamePath(instantFileNameGenerator.getFileName(toInstant)), writerOption);
        } else {
          storage.createImmutableFileInPath(getInstantFileNamePath(instantFileNameGenerator.getFileName(toInstant)), writerOption);
        }
  ...
  public void saveToPendingClusterCommit(HoodieInstant instant, Option<HoodieInstantWriter> writerOption) {
    // In 0.x, no separate clustering action, reuse replace action.
    saveToPendingReplaceCommit(instant, writerOption);
  }
