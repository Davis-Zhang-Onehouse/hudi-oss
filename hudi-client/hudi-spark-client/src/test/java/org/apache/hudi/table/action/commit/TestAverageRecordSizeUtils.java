...
public class TestAverageRecordSizeUtils {
  ...
  public void testAverageRecordSize(List<Pair<HoodieInstant, List<HWriteStat>>> instantSizePairs, long expectedSize) throws IOException {
    ...
    instantSizePairs.forEach(entry -> {
      ...
      try {
        when(mockTimeline.getInstantContentStream(hoodieInstant)).thenReturn(Option.of(
            new ByteArrayInputStream(TimelineMetadataUtils.serializeCommitMetadata(COMMIT_METADATA_SER_DE, commitMetadata).get())));
      } catch (IOException e) {
