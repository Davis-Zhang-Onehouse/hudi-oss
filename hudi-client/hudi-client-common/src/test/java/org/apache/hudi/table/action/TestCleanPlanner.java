...
public class TestCleanPlanner {
  ...
  void testGetDeletePaths(HoodieWriteConfig config, String earliestInstant, List<HoodieFileGroup> allFileGroups, List<Pair<String, Option<HoodieInstantWriter>>> savepoints,
    ...
    if (!savepoints.isEmpty()) {
      ...
      for (Pair<String, Option<HoodieInstantWriter>> savepoint : savepoints) {
        ...
        when(activeTimeline.getInstantDetails(instant)).thenReturn(savepoint.getRight().map(Transformations::writeInstantContentToBytes));
  ...
  void testPartitionsForIncrCleaning(boolean isPartitioned, HoodieWriteConfig config, String earliestInstant,
    ...
    if (!savepoints.isEmpty()) {
      for (Map.Entry<String, List<String>> entry : savepoints.entrySet()) {
        Pair<HoodieSavepointMetadata, Option<HoodieInstantWriter>> savepointMetadataOptionPair = getSavepointMetadata(entry.getValue());
        ...
        when(activeTimeline.getInstantDetails(instant)).thenReturn(savepointMetadataOptionPair.getRight().map(Transformations::writeInstantContentToBytes));
      }
    }

    // prepare last Clean Metadata
    Pair<HoodieCleanMetadata, Option<HoodieInstantWriter>> cleanMetadataOptionPair =
  ...
  static Stream<Arguments> keepLatestVersionsArgs() {
    ...
    List<Pair<String, Option<HoodieInstantWriter>>> savepoints = Collections.singletonList(Pair.of(instant4, getSavepointBytes("partition1", Collections.singletonList(instant4Path))));
    arguments.add(Arguments.of(
        keepLatestVersionsConfig,
        instant1,
        Collections.singletonList(fileGroup),
    ...
    List<Pair<String, Option<HoodieInstantWriter>>> replacedFileGroupSavepoint = Collections.singletonList(
        Pair.of(instant4, getSavepointBytes("partition1", Collections.singletonList(replacedFilePath))));
    arguments.add(Arguments.of(
        keepLatestVersionsConfig,
        instant1,
        Collections.singletonList(buildFileGroup(Arrays.asList(instant2, instant1))),
  ...
  static Stream<Arguments> keepLatestByHoursOrCommitsArgs() {
    ...
    List<Pair<String, Option<HoodieInstantWriter>>> savepoints = Collections.singletonList(Pair.of(oldestFileInstant, getSavepointBytes("partition1", Collections.singletonList(oldestFilePath))));
    arguments.addAll(buildArgumentsForCleanByHoursAndCommitsCases(
        earliestInstant,
        Collections.singletonList(fileGroup),
        savepoints,
    ...
    List<Pair<String, Option<HoodieInstantWriter>>> savepointsForReplacedGroup = Collections.singletonList(Pair.of(oldestFileInstant,
  ...
  private static List<Arguments> buildArgumentsForCleanByHoursAndCommitsCases(String earliestInstant, List<HoodieFileGroup> allFileGroups, List<Pair<String, Option<HoodieInstantWriter>>> savepoints,
  ...
  private static Option<HoodieInstantWriter> getSavepointBytes(String partition, List<String> paths) {
    ...
    return getInstantWriter(savepointMetadata);
  }

  private static Pair<HoodieCleanMetadata, Option<HoodieInstantWriter>> getCleanCommitMetadata(List<String> partitions, String instantTime, String earliestCommitToRetain,
    ...
    return Pair.of(cleanMetadata, getInstantWriter(cleanMetadata));
  }

  private static Pair<HoodieSavepointMetadata, Option<HoodieInstantWriter>> getSavepointMetadata(List<String> partitions) {
    Map<String, HoodieSavepointPartitionMetadata> partitionMetadata = new HashMap<>();
    ...
    HoodieSavepointMetadata savepointMetadata =
        new HoodieSavepointMetadata("user", 1L, "comments", partitionMetadata, 1);
    return Pair.of(savepointMetadata, getInstantWriter(savepointMetadata));
  }
  ...
  private static HoodieCleanerPlan mockLastCleanCommit(HoodieTable hoodieTable, String timestamp, String earliestCommitToRetain, HoodieActiveTimeline activeTimeline,
                                                       Pair<HoodieCleanMetadata, Option<HoodieInstantWriter>> cleanMetadata, Set<String> savepointsTrackedInLastClean)
      throws IOException {
    ...
    when(activeTimeline.getInstantDetails(latestCleanInstant)).thenReturn(cleanMetadata.getRight().map(Transformations::writeInstantContentToBytes));
    ...
    when(activeTimeline.readCleanerInfoAsBytes(any())).thenReturn(getInstantWriter(cleanerPlan).map(Transformations::writeInstantContentToBytes));
    ...
    instantstoProcess.forEach((k, v) -> {
      ...
      try {
        when(hoodieTable.getActiveTimeline().getInstantDetails(hoodieInstant)).thenReturn(
            Option.of(commitMetadata).map(Transformations::writeInstantContentToBytes));
      } catch (Exception e) {
